...
public class AnswerLayout {
    ...
    private static final List<String> recordList = new ArrayList<>(); //答题记录
    ClockDisplay mt;
    ...
    public AnswerLayout() {
        ...
        nextButton1.addActionListener(e -> {
            if (totalProblemNum != remainingProblemNum) {
                addRecord(questionField.getText(), String.valueOf(correctAnswer), answerField.getText());
            }
            answerField.setText("");
            if (remainingProblemNum == 0) {//判断答题是否结束
                //将答题记录写入文件
                new Record().save(new ArrayList<>(recordList));
                ...
                //启动倒计时器
                if (remainingProblemNum == totalProblemNum && isExam) {
    ...
    // 记录一条做题信息
    private void addRecord(String question, String correctAnswer, String yourAnswer) {
